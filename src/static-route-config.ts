    ...
    KaliStaticConfigBuilder<State, Context, Query, Headers, Params, Consumed>
{
  ...
  public headers<Headers>(headers: Validator<Headers, Record<string, string>>) {
    ...
    return this as unknown as NextKaliStaticConfigBuilder<
      State,
      Context,
      Query,
      Headers,
  ...
  public serve(options: StaticOptions): State {
    ...
    this.config.redirectNotFoundToIndex =
      options.redirectNotFoundToIndex ?? false;
    this.config.rangeSize = options.rangeSize;
    this.config.highWaterMark = options.highWaterMark;

    this.app.get(
      `${this.pattern}*`,
      createStaticRouteHandler(this.context, this.pattern, this.config),
    );
